...
export function ListPage() {
  ...
  const hasTable = !!list?.tableColumns && list.items.some((item) => item.meta);

  const filteredItems = useMemo(() => {
    if (!list) return [];
    if (!activeFilter) return list.items;
    return list.items.filter(
      (item) => item.meta?.[activeFilter.key] === activeFilter.value
    );
  }, [list, activeFilter]);
  ...
  const getColumnLabel = (key: string) => {
    return list?.tableColumns?.find((c) => c.key === key)?.label || key;
  };
  const getColumnEmoji = (key: string) => {
    return list?.tableColumns?.find((c) => c.key === key)?.emoji || "";
  };
        ...
        <div className="flex flex-wrap gap-2">
          {allLists
            ...
            .map((l) => (
              ...
              >
                {l.emoji}{" "}
                {l.title.replace(/^\d+\s/, "").replace(/^Best\s/, "")}
              </Link>
            ))}
        </div>
      </motion.div>
